...
enyo.kind({
	...
	getVideos: function() {
		...
		this.$.videoService.call({params: params});
	},
				...
				if (video.size > this.mmsMaxVideoSize) {
					...
					if (this.category == "captured") {
						...
						if (video.duration <= this.mmsTranscodedDuration) {
							...
							this.$.videoEdit.setContent($L("Trim and send"));
	...
	videoPictureUrlFormatter: function(artUrl){
		...
		if (artUrl.thumbnails[0])
			...
			result = enyo.path.rewrite("$enyo-lib/systemui/images/thumb-video.png");
	...
	videoDurationFormatter: function(duration){
		...
		// this looks funny, but leaving it in case HI goes back to wanting seperate strings
		var hourSingle = $L(" hr");
		var hourPlural = $L(" hr");
		var minSingle = $L(" min");
		var minPlural = $L(" min");
