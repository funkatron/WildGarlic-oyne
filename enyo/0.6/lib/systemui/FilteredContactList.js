...
enyo.kind({
	...
	renderList: function() {
		...
		if (!this.height && n && ln) {
			...
			n.style.maxHeight = (document.body.offsetHeight - enyo.dom.fetchNodeOffset(ln).offsetTop - this.offsetBottom) + "px";
		}
	},
	...
	getContacts: function(inQuery, inSearch) {
		...
		this.$.getContacts.call({method: method, params: {query: q}});
	},
