...
enyo.kind({
	...
	chrome: [
		{name: "client", className: "enyo-popup-inner", kind: "BasicScroller", components: [
	...
	// NOTE: PopupList assumes items are strings; however, it's valid in its superclass PopupMenu
	// for items to be objects.
	listGetItem: function(inSender, inIndex) {
		var l = this.items.length;
		if (inIndex < l) {
			this.$.item.setCaption(this.items[inIndex]);
	...
	scrollToSelected: function() {
		var n = this.fetchRowNode(this.selected);
		if (n) {
			this.scrollIntoView(n ? n.offsetTop : 0);
		}
	}
});