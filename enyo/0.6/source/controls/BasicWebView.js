...
enyo.kind({
	...
	rendered: function() {
		...
		if (this.hasNode()) {
			...
			if (this.frameworkClick) {
				this.singleTapHandler = enyo.bind(this, "singletapHandler");
				this.node.addEventListener("singletap", this.singleTapHandler, false);
				this.domAttributes["enyo-pass-events"] = false;
			}
	...
	singletapHandler: function(inEvent) {
		var o = enyo.dom.fetchNodeOffset(this.node);
		this.callBrowserAdapter("clickAt", [inEvent.centerX - o.offsetLeft, inEvent.centerY - o.offsetTop, 1]);
	},
	...
	smartZoomAreaFound: function(left, top, right, bottom, centerX, centerY, spotlightHandle) {
		if (this.accelerated) {
			this.smartZoomAreaFoundAccelerated(left, top, right, bottom, centerX, centerY, spotlightHandle);
		} else {
			this.smartZoomAreaFoundUnaccelerated(left, top, right, bottom, centerX, centerY, spotlightHandle);
		}
	},
	smartZoomAreaFoundUnaccelerated: function(inLeft, inTop, inRight, inBottom, inCenterX, inCenterY, inSpotlightHandle) {
		var zoom = this._boxSize.w / (inRight - inLeft);
		var scale = zoom / this.zoom;
		var left;
		var s = this.fetchScrollPosition();
		if (zoom == this.zoom) {
		...
		var top = inCenterY * zoom - this._boxSize.h / 2;
		if (top < 0) {
			top = 0;
		}
		...
		this._s = {
			zoom: (zoom - this._c.zoom) / steps,
			left: (left - this._c.left) / steps,
			top: (top - this._c.top) / steps
		};
		this.callBrowserAdapter("enableFastScaling", [true]);
	...
	smartZoomAreaFoundAccelerated: function(inLeft, inTop, inRight, inBottom, inCenterX, inCenterY, inSpotlightHandle) {
		var zoom = this._boxSize.w / (inRight - inLeft);
		var left;
		if (zoom == this.zoom) {
			// zoom back to default
		...
		this._smartZoomInfo = {
			...
			left: left,
			top: top,
			scrollChanged: left != s.l || top != s.t
		};
	},
	webkitTransitionEndHandler: function() {
		...
		this.callBrowserAdapter("skipPaintHack", [z.zoomChanged, z.scrollChanged, -z.left + o.offsetLeft + s.l, -z.top + o.offsetTop + s.t]);
