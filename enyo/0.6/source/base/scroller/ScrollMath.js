	...
	*/
	...
	animate: function() {
		...
		var fn = enyo.hitch(this, function() {
			...
			t0 = t1;
			// slop factor of up to 3ms is observed in Chrome (?)
			/*
			if (dt < this.interval - 3) {
			...
			*/
			...
			t += dt;
			// we don't expect t to be less than frame, unless the interval was very much
			// less than expected (which does occur on device)
			if (t < this.frame) {
				return;
			}
