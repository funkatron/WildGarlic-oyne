...
enyo.windows.manager = {
	...
	isValidWindow: function(inWindow) {
		...
		return Boolean(inWindow && !inWindow.closed && (!inWindow.PalmSystem || inWindow.PalmSystem));
	},
	addWindow: function(inWindow, inRecordActivate) {
		var windows = this.getWindows();
		windows[inWindow.name] = inWindow;
		if (inRecordActivate) {
			this.recordActiveWindow(inWindow);
		}
	},
	...
	getActiveWindow: function() {
		var app = this.getRootWindow();
		var am = app.enyo.windows.manager;
		var n = am._activeWindow;
		return this.fetchWindow(n);
	},
	recordActiveWindow: function(inName) {
		var w = this.getRootWindow();
		w.enyo.windows.manager._activeWindow = inName;
	},
...
enyo.addOnStart(function() {
	...
	m.addUnloadListener(window);
	m.addLoadListener(window);
	//
