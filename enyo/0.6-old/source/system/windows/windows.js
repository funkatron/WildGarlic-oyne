...
launches the running application or a system service does so.
In either case, new window params may be sent to the application. An
application can customize its response to being relaunched based on this information by implementing
the applicationRelaunch event as follows:
...
enyo.windows = {
	...
	*/
	openWindow: function(inUrl, inName, inParams, inAttributes, inWindowInfo) {
		...
		var w = this.agent.open(root, inUrl, inName || "", attributes, inWindowInfo);
		this.finishOpenWindow(w, inParams, attributes);
		return w;
	},
	//* @protected
	...
	finishOpenWindow: function(inWindow, inParams, inAttributes) {
		inWindow.name = inWindow.name || enyo.windows.makeUniqueWindowName()
		this.assignWindowParams(inWindow, inParams);
		// tell manager about new window
		// record this as the active window only if it's a card.
		var recordAsActive = (inAttributes && inAttributes.window == "card");
		this.manager.addWindow(inWindow, recordAsActive);
	},
	...
	*/
	...
	/**
	Send paramaters to the given window. Application windows can communicate by sending window
	...
	The window specified by inWindow will fire a windowParamsChange event which can be implemented
	...
	* inParams {Object} Parameters to send to the window. This object will be made available 
	as enyo.windowParams.
	*/
	setWindowParams: function(inWindow, inParams) {
		this.assignWindowParams(inWindow, inParams);
		this.events.dispatchWindowParamsChange(inWindow);
	},
	//* @protected
	// sets window params silently, no event.
	assignWindowParams: function(inWindow, inParams) {
		var params = inParams && enyo.isString(inParams) ? enyo.json.from(inParams) : inParams || {};
