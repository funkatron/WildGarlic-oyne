...
enyo.kind({
	...
	srcChanged: function() {
		if (this.src && this.bufferImage.src == this.src) {
			...
			this.$.image.setSrc("$base-controls/images/blank.gif");
	...
	adjustSize: function() {
		...
		this.maxZoom = Math.max(this.defaultMaxZoom, this.minZoom * this.defaultMaxZoom);
	...
	gesturestartHandler: function(inSender, e) {
		...
		//this.log(enyo.json.to(this._zoomStart));
	},
	...
	gestureendHanlder: function(inSender, e) {
		enyo.stopEvent(e);
	...
	smartZoom: function(e) {
		...
		var cx = e.clientX - o.offsetLeft, cy = e.clientY - o.offsetTop;
		var p = this.fetchScrollPosition();
	...
	stepAnimation: function(inSender, inValue, inProgress, inSteps) {
